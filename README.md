...
**Important:** To prevent API corruption (Dirty JSON) during AJAX requests, PHP is configured to **suppress all errors from the browser output**.

* **You will NOT see errors on the screen.** A crash will usually result in a generic "Success" message or a blank behavior.
* **Deprecation Warnings** are suppressed to reduce noise on newer PHP versions.

**How to view errors:**
You must view the container logs to see PHP errors, warnings, or exceptions:
...
| `make dev-logs` | Tail logs for all containers (App, Cron, DB, Web). **Required for debugging.** |
...
* `config/` - Configuration files mounted into containers:
    ...
    * `php.custom.ini` - PHP overrides. **Errors are suppressed here.**
...
*Note: `display_errors` is set to `Off` and `error_reporting` filters out `E_DEPRECATED` in this file to prevent JSON corruption.*
